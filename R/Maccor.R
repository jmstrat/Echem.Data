...
load.maccor <- function(file) {
  ...
  if("Current.A." %in% names(data))
    data$Current.A. <- sapply(data$Current.A.,function(x){as.numeric(gsub(",", "", x))})
  if("Test_Time.s." %in% names(data))
    data$Test_Time.s.<- sapply(data$Test_Time.s.,function(x){as.numeric(gsub(",", "", x))})
  if("Step_Time.s." %in% names(data))
    data$Step_Time.s.<- sapply(data$Step_Time.s.,function(x){as.numeric(gsub(",", "", x))})
  if("Voltage.V." %in% names(data))
    data$Voltage.V.<- sapply(data$Voltage.V.,function(x){as.numeric(gsub(",", "", x))})
  ...
  #Adjust step indexes
  data$Step_Index=data$Step_Index-2

  #Adjust cycle indexes (for rest at start)
  data$Cycle_Index[data$Cycle_Index<1]<-1
